...
final class LocationStateManager: NSObject, CLLocationManagerDelegate, ObservableObject {
    ...
    public var currentLocation: CLLocation? {
        if let live = lastLocation { return live }
        // Fall back to cached location (survives airplane mode)
        let lat = storage.double(forKey: cachedLatKey)
        let lon = storage.double(forKey: cachedLonKey)
        if lat != 0.0 || lon != 0.0 {
    ...
    private let cachedLatKey = "emergencySOS.lastLatitude"
    private let cachedLonKey = "emergencySOS.lastLongitude"
    ...
    /// Kick off a fresh location request and deliver the best available
    /// coordinate via `completion` within `timeout` seconds.
    /// Priority: live fix > UserDefaults cache > nil
    public func fetchLocationForEmergency(timeout: TimeInterval = 3.0, completion: @escaping (CLLocationCoordinate2D?) -> Void) {
        // If we already have a recent live fix (< 30 seconds old), use it immediately
        if let loc = lastLocation, Date().timeIntervalSince(loc.timestamp) < 30 {
            print("📍 SOS using live GPS: \(loc.coordinate.latitude), \(loc.coordinate.longitude)")
        ...
        // Request a fresh fix
        cl.requestLocation()
        print("📡 SOS: requesting fresh GPS fix (timeout \(timeout)s)...")
        
        var delivered = false
        
        // Observer: fires when didUpdateLocations sets lastLocation
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + timeout) {
            ...
            // Check cache
            let lat = self.storage.double(forKey: self.cachedLatKey)
            let lon = self.storage.double(forKey: self.cachedLonKey)
            if lat != 0.0 || lon != 0.0 {
                print("📍 SOS timeout — using cached GPS: \(lat), \(lon)")
                completion(CLLocationCoordinate2D(latitude: lat, longitude: lon))
            } else {
                print("⚠️ SOS: No GPS available — sending without coordinates")
                completion(nil)
            }
        }
        
        // Poll for a new location every 0.2s
        var attempts = 0
        let maxAttempts = Int(timeout / 0.2)
        func poll() {
            guard !delivered else { return }
            if let loc = self.lastLocation, Date().timeIntervalSince(loc.timestamp) < 5 {
                delivered = true
                observation?.invalidate()
                print("📍 SOS got fresh GPS: \(loc.coordinate.latitude), \(loc.coordinate.longitude)")
                completion(loc.coordinate)
                return
            }
            attempts += 1
    ...
    func locationManager(_ manager: CLLocationManager, didUpdateLocations locations: [CLLocation]) {
        ...
        // Cache for airplane mode SOS — persist every GPS fix
        storage.set(loc.coordinate.latitude, forKey: cachedLatKey)
        storage.set(loc.coordinate.longitude, forKey: cachedLonKey)
