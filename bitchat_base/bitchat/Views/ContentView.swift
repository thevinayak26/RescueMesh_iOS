...
struct ContentView: View {
    ...
    @ObservedObject private var emergencySensor = EmergencySensorService.shared
...
// MARK: - Body
    ...
    var body: some View {
        ...
        // SOS Countdown Overlay — appears on top of everything
        if emergencySensor.isCountdownActive || emergencySensor.sosSent {
            SOSCountdownOverlay()
        }
        } // end ZStack
    ...
    private var mainHeaderView: some View {
        HStack(spacing: 0) {
            ...
            // SOS Button (starts countdown)
            Button(action: {
                EmergencySensorService.shared.triggerManualSOS()
            }) {
                HStack(spacing: 4) {
            ...
            // Force SOS Button (instant, no countdown — for demo)
            ForceSOSButton()
                .padding(.trailing, 6)
