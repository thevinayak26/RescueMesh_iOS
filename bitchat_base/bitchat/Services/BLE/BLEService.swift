...
final class BLEService: NSObject {
    ...
    private struct OutboundPriority: Comparable {
        ...
        static let emergency = OutboundPriority(level: -1, suborder: 0)
...
extension BLEService {
    ...
    private func priority(for packet: BitchatPacket, data: Data) -> OutboundPriority {
        // Detect Emergency Payload
        if packet.type == MessageType.message.rawValue {
            if let payloadString = String(data: packet.payload, encoding: .utf8), payloadString.hasPrefix("EMERGENCY_SOS") {
                return .emergency
            }
        }
