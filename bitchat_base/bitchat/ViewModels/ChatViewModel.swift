...
final class ChatViewModel: ObservableObject, BitchatDelegate, CommandContextProvider, GeohashParticipantContext, MessageFormattingContext {
    ...
    init(
        ...
        setupEmergencySensor()
        
        // Start standalone SOS GPS tracker (continuous updates, caches to UserDefaults)
        SOSLocationManager.shared.startTracking()
...
// MARK: - Emergency Extensions
extension ChatViewModel {
    private func setupEmergencySensor() {
        EmergencySensorService.shared.emergencyEventPublisher
            .receive(on: DispatchQueue.main)
            .sink { [weak self] type in
                self?.handleEmergencyEvent(type: type)
            }
            .store(in: &cancellables)
            
        EmergencySensorService.shared.startMonitoring()
        
        // Also listen for Decision Engine fallbacks directly if triggered by something else
        NotificationCenter.default.addObserver(forName: NSNotification.Name("BitChatEmergencyFallbackTriggered"), object: nil, queue: .main) { [weak self] note in
            if let payload = note.userInfo?["payload"] as? String {
                Task { @MainActor in
                    self?.broadcastEmergencyPayload(payload)
                }
            }
        }
    }
    
    private func handleEmergencyEvent(type: EmergencyType) {
        print("ChatViewModel received Emergency: \(type)")
        
        // Use the dedicated SOS GPS tracker — always has the latest coordinate
        let coordinate = SOSLocationManager.shared.getBestCoordinate()
        let location = coordinate ?? CLLocationCoordinate2D(latitude: 0.0, longitude: 0.0)
        print("📍 SOS broadcasting with coordinates: \(location.latitude), \(location.longitude)")
        CommunicationDecisionEngine.shared.evaluateAndExecute(for: type, location: location)
    }
    ...
    private func checkForSOS(_ message: BitchatMessage) {
        guard message.sender != nickname else { return }
        
        if message.content.hasPrefix("EMERGENCY_SOS") {
            if let packet = EmergencyPacket.decodeFromPayload(message.content) {
                SecureLogger.info("🚨 SOS Received from \(message.sender)", category: .session)
                NotificationService.shared.sendEmergencyNotification(packet: packet, from: message.sender)
                
                // Also trigger a haptic alert immediately
                #if os(iOS)
